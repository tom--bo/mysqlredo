...
static byte *parse_index_versioned_fields(byte *ptr, const byte *end_ptr,
  ...
  for (auto i = n_inst; i > 0; --i) {
    ...
    if (ptr == nullptr) return (nullptr);
    std::cout << ", n[i]: " << i << ", logical_pos: " <<  info.logical_pos << ", phy_pos: " << info.phy_pos;
    ...
    if ((info.phy_pos & 0x4000) != 0) {
      ...
      n_dropped++;
      std::cout << ", v_dropped: " << info.v_dropped;
    }
...
byte *mlog_parse_index(byte *ptr, const byte *end_ptr, dict_index_t **index) {
  ...
  std::cout << ", index_log_ver: " << uint32_t(index_log_version) << ", index_flag: " << uint32_t(flag);
  /* Read n and n_uniq */
  uint16_t n = 0;
  uint16_t n_uniq = 0;
  ...
  std::cout << ", cols: " << (uint16_t)n << ", inst_cols: " << (uint16_t)inst_cols << ", uniq_cols: " << (uint16_t)n_uniq;
